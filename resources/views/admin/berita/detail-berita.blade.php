            ...
            <div class="card mb-3">
                <div class=" carousel slide carousel-fade" id="controlStyledExample" data-ride="carousel">
                    <div class="carousel-indicators">
                        @foreach ($post->multipleImage as $i => $image)
                            @if ($i == 0)
                                <button class="active" type="button" data-bs-target="#controlStyledExample" data-bs-slide-to="{{ $i }}" aria-current="true" aria-label="Slide {{ $i }}"></button>
                            @else
                                <button type="button" data-bs-target="#controlStyledExample" data-bs-slide-to="{{ $i }}" aria-current="true" aria-label="Slide {{ $i }}"></button>
                            @endif
                        @endforeach
                ...
                <div class="card-body">
                    <div class="row justify-content-between align-items-center">
                        <div class="col">
                            <div class="d-flex">
                                <div class="calendar me-2"><span class="calendar-month">{{ $post->created_at->isoFormat('MMM') }}</span><span class="calendar-day">{{ $post->created_at->isoFormat('D') }} </span>
                                </div>
                        ...
                        <div class="col-md-auto mt-4 mt-md-0">
                            <button class="btn btn-falcon-default btn-sm me-2" type="button"><span class="fas fa-heart text-danger me-1"></span>{{ $post->like }}</button>
